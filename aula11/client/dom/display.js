class Display {
    ...
    clearForm() {
        const arr = ["mes", "tipo", "valor", "categoria"]
    ...
    formAction() {
        ...
        fetch("http://localhost:3000/api/lancamentos", { method: "post", headers: { "content-type": "application/json" }, body: JSON.stringify({ mes: data[0], categoria: toTitleCase(data[3]), tipo: data[1].toLowerCase(), valor: Number(data[2]) }) })
        this.year.calcularSaldo(); 
